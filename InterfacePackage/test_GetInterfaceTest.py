...
get_url = base_url + endpoint#请求接口信息拼接放在一个变量中，方便后续测试用例统一调用

# 创建请求头，请求头放在一个字典变量中，方便后续测试用例统一调用
...
def test_TestCase1_youxiaoyonghu():
    print("\n执行测试用例1：查询有效用户")#执行用例前打印一个提示信息，没有业务作用，只是为了执行用例时，用来看当前要执行那条用例而已

    # 准备请求数据（测试步骤的数据），使用json模块中的json.dumps()将Python 对象（字典、列表等）转换为 JSON 格式的字符串
    # 因为后续我们要使用http请求，HTTP 请求的正文（body）通常需要 JSON 字符串格式
    rucan = json.dumps({
    ...
    # 发送请求
    # 使用预先配置好的session发送请求
    # 因为我们这里使用了session，加强了连接的会话稳定性；如果不使用的话，就正常使用http请求的requests.get或者requests.post
    # get请求和post请求都有对应的参数标准：这里面post请求中几个参数：
    # get_url正常传字符串数据
    # data直接传递 JSON 字符串数据
    # headers传字典格式数据
